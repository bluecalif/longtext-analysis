...
**목표**: 마크다운 파싱 로직의 정확도와 안정성 향상

**현재 문제**:
- `docs/longcontext_phase_2.md` 파일 파싱 시 여러 문제 발견
- Turn 분리 로직이 복잡한 마크다운 구조에서 정확하게 동작하지 않음
- 코드 블록 추출이 누락되거나 잘못 추출되는 경우 발생
- `turn_index 12`에서 너무 많은 내용이 한꺼번에 묶이는 문제

**⚠️ 발견된 추가 문제** (2025-12-29):
1. **Body에서 코드 블록 제거 로직 미작동**
   - 현재 `parse_turn()`에서 `re.sub(r"```.*?```", "", block, flags=re.DOTALL)` 사용
   - 단순 정규식으로는 중첩 블록, 여러 백틱 개수 등을 제대로 처리하지 못함
   - 결과: body에 코드 블록 내용이 여전히 포함됨
   - **해결 필요**: `extract_code_blocks()`로 추출한 코드 블록을 body에서 정확히 제거

2. **코드 블록 추출 로직의 백틱 인식 문제**
   - 현재 `extract_code_blocks()`가 백틱 3개 이상(```)으로 감싸진 것만 추출해야 함
   - 하지만 모든 내용이 다 추출되는 것으로 보임
   - **해결 필요**: 코드 펜스 패턴(백틱 3개 이상)을 정확히 인식하도록 개선

**개선 계획**:
1. **Turn 분리 로직 단순화 및 개선** ✅
   - `---` 구분선 + `**User**`/`**Cursor**` 패턴 기반 분리
   - 코드 블록 보호 로직 개선
   - Speaker 라벨 인식 정확도 향상

2. **코드 블록 추출 로직 개선: 중첩 블록 처리** ✅
   - 중첩 레벨(nesting_level) 추적 로직 추가 완료
   - 언어태그 없는 ``` 처리 규칙 구현 완료:
     ...
     - nesting_level = 0이면:
       ...
       - 다음 ```가 없으면 → 외부 블록 종료
   - 언어태그 있는 펜스는 항상 시작 펜스 (nesting_level++)
   - 줄 단위 파싱으로 안정성 향상 완료
   - 코드 펜스 시작/종료 패턴 명확화 완료
   - **테스트 완료**: `tests/test_nested_code_blocks.py` 모든 케이스 통과 (6/6)

3. **Body에서 코드 블록 제거 로직 개선** ✅
   - `extract_code_blocks_with_positions()`에 위치 정보(시작/종료 라인 인덱스) 추가 완료
   - `parse_turn()`에서 추출된 코드 블록의 위치 정보를 활용하여 body에서 정확히 제거 완료
   - 코드 펜스 시작/종료 라인을 포함하여 제거 완료

4. **파싱 품질 검증 강화**
   - Unknown speaker 비율 20% 미만 유지
   - 코드 블록 추출 개수 검증
   - Turn 분리 정확도 검증
...
- ✅ Body에서 코드 블록 제거 로직 개선 완료
  - `extract_code_blocks_with_positions()`에 위치 정보 추가 완료
  - `parse_turn()`에서 위치 정보 활용하여 body에서 제거 완료
- ✅ 코드 블록 추출 로직 개선: 중첩 블록 처리 완료
  - 중첩 레벨 추적 로직 구현 완료
  - 언어태그 없는 ``` 처리 규칙 구현 완료
  - look-ahead 로직 구현 완료
...
- ✅ **중첩 코드 블록 테스트**: 모든 케이스 통과 (6/6)
  - 케이스 1: 단일 코드 블록 ✅
  - 케이스 2: 병렬 코드 블록 (언어태그 있음) ✅
  - 케이스 2-1: 병렬 코드 블록 (언어태그 없음 → 중첩) ✅
  - 케이스 3: 중첩 코드 블록 ✅
  - 케이스 4: 병렬 + 중첩 코드 블록 ✅
  - 케이스 5: 중첩 안에 병렬 (언어태그 없는 경우 포함) ✅
- ✅ **실제 마크다운 파일 파싱 테스트**: 두 파일 모두 통과
  - `longcontext_phase_2.md`: 39 turns, Unknown 2.56%, Code blocks 66개, Body 정리 100% ✅
  - `longcontext_phase_4_3.md`: 65 turns, Unknown 1.54%, Code blocks 140개, Body 정리 100% ✅

**수정된 파일**:
- `backend/parser/turns.py`: Turn 분리 로직 개선
- `backend/parser/snippets.py`: 코드 블록 추출 로직 개선 (중첩 블록 처리, 위치 정보 추가)
  - `extract_code_blocks_with_positions()`: 중첩 레벨 추적 및 look-ahead 로직 구현
  - 언어태그 없는 ``` 처리 규칙 구현
- `tests/test_nested_code_blocks.py`: 중첩 코드 블록 테스트 케이스 추가
- `tests/fixtures/nested_code_blocks_test.md`: 테스트 케이스 및 groundtruth 정의
- `tests/test_longcontext_parsing.py`: E2E 테스트 (phase_2, phase_4_3 두 파일 모두 테스트)
- `scripts/check_body_code_blocks_phase_4_3.py`: Body 검증 스크립트 추가
...
**목표**: README.md와 커서룰을 보강하여 프로젝트 문서화를 완성합니다.

**완료된 Sub Phase**:
- ✅ Phase 12.1: README.md 업데이트
  - 실행 방법 섹션 개선 (프론트엔드 구현 완료 반영, 스크립트 사용법 추가)
  - 빠른 시작 가이드 보강
  - API 엔드포인트 요약 추가
  - 프로젝트 상태 업데이트 (Phase 11 완료 반영)
  - 트러블슈팅 섹션 추가 (포트 충돌, 환경 변수, PowerShell 실행 정책, Poetry 버전)
- ✅ Phase 12.2: 커서룰 보강
  - `.cursor/rules/execution-environment.mdc` 생성 (실행 환경 규칙)
  - 서버 실행 방법, 스크립트 사용법, 포트 충돌 해결 방법 문서화

**주요 업데이트 내용**:
- API 엔드포인트 요약: 8개 주요 엔드포인트 설명 추가
- 트러블슈팅 가이드: 포트 충돌, 환경 변수, PowerShell 실행 정책, Poetry 버전 오류 해결 방법
- 실행 환경 규칙: 커서룰에 실행 환경 관련 규칙 추가

---

## 진행 상황 추적

**현재 Phase**: Phase 12 완료 (문서화 정리)
...
- Phase 8: 프론트엔드 UI 구현 (2025-12-28 완료)
  ...
  - ✅ Phase 8.9: 스타일링 및 UX 개선
  - ✅ Phase 8.10: 통합 테스트 및 검증
- Phase 9: 파싱 개선 (2025-12-30 완료)
  - ✅ Turn 분리 로직 개선 (Speaker 기준 단순화)
  - ✅ 코드 블록 추출 로직 개선 (중첩 블록 처리)
  - ✅ Body에서 코드 블록 제거 로직 개선
  - ✅ `turn_index 12` 문제 해결 완료
