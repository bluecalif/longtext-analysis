...
- README.md: 이 파일을 Phase 진행마다 업데이트해야 코드베이스의 hierarchy와 데이터 flow가 지속적으로 인식됩니다
...
3. 로그 폴더 및 로그 설정이 분석 가능한 수준으로 되어 있는지 확인
...
### 2.5 캐시 사용 여부
**캐시 사용 구조**:
-캐시가 사용되도록 하는 구조 (특히 llm 사용시 비용 감소를 위해) 프로젝트 진행시 커서가 실수를 가장 많이 하는 부분.
보통 캐시 사용하게 작성한다고 하지만, 실제로는 활용안되게 설계되는 경우가 대부분.
-개선 방안: 캐시 사용 여부를 체크되게 (로그나 카운터) 코드 구축. 프로젝트 진행시 커서에게 더블 체크를 요청하기**
- **결정적 해시 함수 사용** : (SHA-256, `hash()` 사용 금지)
- 원자적 파일 저장: 임시 파일 + 원자적 이동

### 2.6 로그 생성 여부
**로그 파일 저장 경우**
로그의 생성은 모든 e2e 테스트의 기본인데 confest에서부터 이것이 적용되어서 모든 테스트에 적용되어야 함
그리고 로그를 터미널에만 출력하지 말고 반드시 파일로 출력하여, 언제든지 재확인 가능하게 할 것
**conftest.py만 실행 가능. conftest_e2e등 파일 이름이 다르면 절대 안됨**

### 2.7 서버 실행
**테스트시 서버 초기화**
- 테스트 실행시마다, 초기에 기존 서버 종료후 재실행하도록 구성

### 2.8 파싱 로직 설정
**기본 전제**
- 커서는 복잡한 형태의 참고 문서를 스스로 파싱 로직을 개발할 수 없다
- 이때는, 반드시 사용자가 원본 문서를 분석->적절한 파싱 로직 검토(by Human)->테스트 및 검증. 이런 절차를 밟아야 한다
**원본 문서 분석**
- 원본 문서의 전체 구조를 파악한다
- 문서의 각 부분을 분리되는 구분자를 파악한다
**파싱 로직 추출**
- 문서의 구분자에 맞추어 문서를 해체하는 방법을 검토(by user)
- 해체한 각 부분을 맥락에 맞게 그룹핑하는 방법을 검토(by user)
- 이 방법들을 커서에게 하나의 로직으로 구성할 것을 지시
**테스트 및 검증**
- 실제로 로직이 성립하는지 테스트
- 필요시 중요 구조는 동일한, 간단한 테스트 문서를 작성해서 테스트 할 것
- 결과를 검증하고, 개선 로직으로 피드백 
